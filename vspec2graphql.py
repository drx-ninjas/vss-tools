...
def print_message_body(nodes, graphql_file):
    for i, node in enumerate(nodes, 1):
        ...
        if node.is_branch() and len(node.children) == 0:
            ...
            graphql_file.write(indent * 2 + "description: String\n")
            graphql_file.write(indent * 2 + "type: String\n")

            if node.min:
                graphql_file.write(indent * 2 + "min: String\n")
            if node.max:
                graphql_file.write(indent * 2 + "max: String\n")
            # enum not included yet 
            # if node.enum:
            #    graphql_file.write(indent * 2 + "enum: String\n")
            
            # changing types "unknown", "0" --> clarification necessary 
            if node.default_value:
                graphql_file.write(indent * 2 + "default: String\n")
            if node.deprecation:
                graphql_file.write(indent * 2 + "deprecation: String\n")
            
            # node.unit call not available? 
            # if node.unit:
            #    graphql_file.write(indent * 2 + "unit: String\n")
            
            graphql_file.write(indent * 2 + "uuid: String\n\n")
